...
exports.is_restart = function(message)
{
	message = clean(message);
	return false ||
		message.match(/^restart/i) ||
		message.match(/^start over/i);
}
...
{
	message = clean(message);
	if(message.match(/\bmale\b/i) || message.match(/^m$/i))
	{
		return 'male';
	}
	if(message.match(/\bfemale\b/i) || message.match(/^f$/i))
	{
		return 'female';
	}
