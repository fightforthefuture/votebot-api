    ...
    */
    {
        ...
        process: function(user, callback) {
            ...
            var electionDay = moment(config.election.date, 'YYYY-MM-DD');
            if (!moment().isSame(electionDay.subtract(1, "days"), 'day')) {
                log.notice('    - Not the day before the election day...');
                return callback({});
            }
            ...
            if (!time.isBetween(startTime, endTime)) {
                log.notice('    - Time is not between 9am PST and 8pm EST! Skipping...');
                return callback({});
            }
