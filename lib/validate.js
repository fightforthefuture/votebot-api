...
exports.time = function(body, user, locale)
{
    // moment with multiple parsers prefers formats earlier in the list
    var formats = [ "h:m a", "h:ma", "h:m", "h a", "ha", "h" ];  
    var time = new moment(body, formats);

    if (time.isValid()) {
        if ((time.hour() < 7) && (body.toUpperCase().indexOf('AM') < 0)) {
            return Promise.reject(data_error(language.template(l10n('error_validate_time_early', locale),
                                             {time: time.format('LT')})
            ));
        }
        // store as 24-hr string
        return Promise.resolve([time.format('H:mm')]);
    } else {
        return Promise.reject(data_error(l10n('error_validate_time', locale)));
    }
};


exports.number = function(body, user, locale) {
    if(body.match(/^[0-9.]*$/)) {
        return Promise.resolve([body]);
    } else {
        return Promise.reject(data_error(l10n('error_validate_number', locale)));
    }
}
...
exports.phone = function(body, options, locale) {
    ...
    if (parsed_phone) {
        ...
        return Promise.reject(data_error(l10n('error_validate_phone', locale)));
    }
}
...
exports.political_party = function(body, user, locale)
{
    ...
    var national_party = party.match(/(democrat|demócrata|republican|libertarian|libertario|green|verde)/)
    if (national_party) {
        return Promise.resolve([national_party[0]]);
    }

    // if other, prompt for it
    if (party === 'other' || party === 'otro') {
...
exports.not_empty = function(body, user, locale)
{
    var text = body.trim();
