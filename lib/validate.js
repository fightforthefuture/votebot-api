...
exports.date = function(body)
{
    ...
    var formats = [ "MM-DD-YYYY", "MM/DD/YYYY", "MMMM DD YYYY", "MMMM DD, YYYY", "MMMM DD YY",  // american month-date
...
{
    // TODO, validate unit number like #2, or apartment 2
    return Promise.resolve([]);
}
...
exports.voter_registration_complete = function(user_settings) {
    ...
    console.info('required_fields', required_fields);

    // remove skippable fields from required_questions
    var skippable_fields = ['state_id', 'ssn_last_4', 'ssn', 'consent_use_signature', 'email'];
    required_fields.filter(function(f) { return (skippable_fields.indexOf(f) > 0); })
    console.info('required_fields w/o skippable', required_fields);
