...
exports.phone = function(body, options) {
    options || (options = {});
    parsed_phone = phone(body, options.country)[0];
    if (parsed_phone) {
        return Promise.resolve([parsed_phone]);
    } else {
        return data_error('Sorry, that wasn\'t a valid phone number. Please format like 510-555-1212');
    }
}
...
exports.military_or_overseas = function(body)
{
    return Promise.resolve([language.military_or_overseas(body), language.is_no(body)])
        .spread(function(military_or_overseas, is_no) {
            ...
            else { throw data_error("Sorry, I didn't get that. Please answer (military/overseas/no)"); }
        });
}
...
exports.state_id_number = function(body, user)
{
    ...
    return data_error(language.template('Please enter a valid {{settings.state}} ID. If you don\'t have one, we can\'t submit your registration. Reply NONE and we\'ll send you a form to print and mail.', user), {promise: true});
}
...
exports.submit_response = function(body, user)
{
    ...
    log.info('validate.submit_response', submit_response);
    return Promise.resolve([submit_response.status === 'queued']);
}
